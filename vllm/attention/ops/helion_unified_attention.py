...
def kernel_helion_v2_attention(
    ...
    # TODO: use query_len tensor instead? might safe retuning/recompilation
    # t_query_lens = torch.diff(t_query_start_lens)
    # max_query_len = t_query_lens.max()
    # rather not hl.specialize?

    q_block_size = hl.register_block_size(4, int(max_query_len))
    # q_block_size = hl.register_block_size(4, max_query_len)
    ...
    for seq_idx, kv_head_idx, qblock_idx in hl.grid([num_seqs, num_kv_heads, max_qblocks]):
        ...
        # cur_qblock_end = torch.maximum(query_start + (qblock_idx + 1) * q_block_size, num_tokens)
        cur_qblock_end = torch.minimum(query_start + (qblock_idx + 1) * q_block_size, query_end)
        ...
        for tile_q, tile_m in hl.tile([cur_qblock_start, kv_head_idx * num_queries_per_kv], 
            ...
            # q = t_query[0:q_block_size, tile_m, :]
            ...
            max_seq_prefix_len = context_len + tile_q.end + (tile_m.block_size + num_queries_per_kv - 1) // num_queries_per_kv
            ...
            for tile_n in hl.tile(num_blocks, block_size=None):
                ...
                # blk_idxs = t_block_tables[seq_idx, 0:tile_n.block_size].view([tile_n.block_size]) #.view(-1)
            ...
            # t_output[0:q_block_size, tile_m, :] = acc.view([tile_q.block_size, tile_m.block_size, head_size])
