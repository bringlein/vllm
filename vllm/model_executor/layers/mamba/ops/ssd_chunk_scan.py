...
@triton_dejavu.jitcache(
    ...
    check_specialization=["hdim", "dstate", "chunk_size", "seqlen", "batch"],
    assume_const=[], # TODO
...
def _chunk_scan_fwd(
    ...
    _chunk_scan_fwd_kernel[grid](
       ...
       # Strides                          
       stride_cb_batch =              cb.stride(0),
       stride_cb_chunk =              cb.stride(1),
       stride_cb_head =               cb.stride(2),
       stride_cb_csize_m =            cb.stride(3),
       stride_cb_csize_k =            cb.stride(4),
       stride_x_batch =               x.stride(0),
       stride_x_seqlen =              x.stride(1),
       stride_x_head =                x.stride(2),
       stride_x_hdim =                x.stride(3),
       stride_z_batch =               z_strides[0],
       stride_z_seqlen =              z_strides[1],
       stride_z_head =                z_strides[2],
       stride_z_hdim =                z_strides[3],
       stride_out_batch =             out.stride(0),
       stride_out_seqlen =            out.stride(1),
       stride_out_head =              out.stride(2),
       stride_out_hdim =              out.stride(3),
       ...
       stride_seq_idx_batch =         seq_idx.stride(0) if seq_idx is not None else 0, 
       stride_seq_idx_seqlen =        seq_idx.stride(1) if seq_idx is not None else 0,
       stride_C_batch =               C.stride(0),
       stride_C_seqlen =              C.stride(1),
       stride_C_head =                C.stride(2),
       stride_C_dstate =              C.stride(3),
       ...
       stride_init_states_batch =     initial_states.stride(0) if initial_states is not None else 0, 
       stride_init_states_head =      initial_states.stride(1) if initial_states is not None else 0,
       stride_init_states_hdim =      initial_states.stride(2) if initial_states is not None else 0,
       stride_init_states_dstate =    initial_states.stride(3) if initial_states is not None else 0,
       stride_D_head =                D.stride(0) if D is not None else 0,
       # Meta-parameters              
       IS_CAUSAL =      True,
       HAS_D =          D is not None,
       D_HAS_HDIM = 2 if D is not None else True,
